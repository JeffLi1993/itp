							...
							.each(
									function() {
										var $input = $(this), inputName = $input
												.attr("name"), inputId = $input
												.attr("id"); // 新加入的ID,为了兼容其他的NAME选择方法
										for ( var key in args) {
											var name = (_lookup.pk == key) ? _util
													.lookupPk(key)
													: _util.lookupField(key);
											var rel = $input.attr("lookupRel");// 新加入的rel,用作共用页面的区分
											if (name == inputId) {
												$input.val(args[key]);
												compareChange(inputName,
														args[key], rel);// 赋值时同时执行'比较'的判断
												break;
											}
											if (name == inputName) {// 兼容其他查找带回
												$input.val(args[key]);
												break;
											}
											...
											.ajax({
												...
												success : function(response) {
													...
													var $lis = $suggest.html(
															'<ul style="width:110px;height:300px;overflow:scroll;" >'
																	+ html
																	+ '</ul>')
															.find("li");
													$lis
								...
								if (addButTxt) {
									var $addBut = $(
											'<div class="button"><div class="buttonContent"><button type="button">'
													+ addButTxt
													+ '</button></div></div><br/><br/><br/>')
											.insertBefore($table)
											.find("button");
									var $rowNum = $(
											'<input type="hidden" name="dwz_rowNum" id="dwz_rowNum" class="textInput" style="margin:2px;" value="1" size="2"/>')
											.insertBefore($table);
									if (saveButTxt.length > 0) {
										var $saveBut = $(
												'<div class="button"><div class="buttonContent"><button type="submit">'
														+ saveButTxt
														+ '</button></div></div>')
												.insertBefore($table);
									}
					...
					function tdHtml(field) {
						...
						case 'lookup':
							...
							html = '<input type="hidden"  name="'
									...
									+ '" lookupRel="'
									+ field.lookupRel
									+ '" readonly="readonly" style = "background-color:#ffffff;" id="'
									+ field.name + '" />';
						...
						default:
							html = '<input type="text" name="'
									+ field.name
									+ '" readonly="readonly" style = "background-color:#ffffff;"  value="'
									+ field.defaultVal
									+ '" size="'
									+ field.size
									+ '" class="'
									...
									+ field.name
									+ '" />'
									+ '<input type="text" name="'// 加入隐藏的date日期控件
									+ field.name
									+ 'Date" readonly="readonly" style = "background-color:#ffffff;display:none;"  value="'
									+ field.defaultVal
									+ '" size="'
									+ field.size
									+ '" class="date"  id="'
									+ field.name
									+ 'Date" />'
									+ '<input type="hidden" value=""  name="" id="lookupId'
									+ field.name
									+ '"/>'// 用来获得查找带回的ID
									...
									+ '<a id="lookupUrl'
									+ field.name
									+ '" class="btnLook" style="display:none;" href="" lookupGroup=""></a>';// 查找带回
							break;
						}
						return '<td>' + html + '</td>';
					}
